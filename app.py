...
# Load dataset
df = pd.read_csv("Startup.csv", encoding="unicode_escape")

# Clean column names
df.columns = df.columns.str.strip()
df = df.rename(columns={"Date dd/mm/yyyy": "Date", "City  Location": "City Location"})

# Clean and convert 'Amount in USD'
df["Amount in USD"] = (
...
selected_city = st.sidebar.selectbox(
    "Select City", options=df["City Location"].dropna().unique()
)
selected_year = st.sidebar.selectbox(
    "Select Year", options=df["Date"].dt.year.dropna().unique()
)

# Filter data
filtered_df = df[
    (df["City Location"] == selected_city) & (df["Date"].dt.year == selected_year)
]
